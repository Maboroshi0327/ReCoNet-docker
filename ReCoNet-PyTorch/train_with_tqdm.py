...
for epoch in range(epochs):
    ...
    for itr, (img1, img2, mask, flow) in enumerate(batch_iterator):
        ...
        content_loss = 0
        content_loss += L2distance(styled_features1[2], img_features1[2].expand(styled_features1[2].size()))
        content_loss += L2distance(styled_features2[2], img_features2[2].expand(styled_features2[2].size()))
        content_loss *= ALPHA / (
        ...
        for i, weight in enumerate(STYLE_WEIGHTS):
            ...
            style_loss += float(weight) * (
                L2distance(gram_img1, gram_s.expand(gram_img1.size()))
                + L2distance(gram_img2, gram_s.expand(gram_img2.size()))
            )
        style_loss *= BETA
    ...
    torch.save(model.state_dict(), "%s/reconet_batch_%d_epoch_%d.pth" % ("runs/output", 1, epoch))
